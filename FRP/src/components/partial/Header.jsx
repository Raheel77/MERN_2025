...
export default function Header({ darkMode, setDarkMode,sidebarToggle, setSidebarToggle }) {
  ...
  ${darkMode ? "bg-gray-900 border-gray-800 text-white" : "bg-white border-gray-200 text-gray-800"}`}>
        <div className="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
            <div className="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 lg:border-b-0 dark:border-gray-800">
                ...
                <button onClick={() => setSidebarToggle(!sidebarToggle)} className="lg:hidden h-10 w-10 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">⋮</button>
            </div>
            <div className={`${sidebarToggle ? "flex" : "hidden"} lg:flex items-center gap-3 p-4`}>
                <button
                ...
                <div className="relative">
                    <button onClick={() => setUserOpen(!userOpen)} className="flex items-center gap-2">
                        <img src="https://i.pravatar.cc/40" alt="user" className="rounded-full"/>Raheel</button>
