import React, {useState, useEffect} from "react";
    import {HomeIcon, UserIcon, UserPlusIcon, UsersIcon,AdjustmentsVerticalIcon, AcademicCapIcon, ChevronDownIcon,} from "@heroicons/react/24/outline";
import {useNavigate, useLocation} from "react-router";
import {BookmarkSquareIcon, BookOpenIcon} from "@heroicons/react/16/solid";
export default function Sidebar() {
    ...
    return (<aside className="w-64 h-screen bg-white border-r border-gray-200 p-4">
        ...
        {openMenu === "teachers" && (
            <div className="ml-8 mt-2 space-y-2">
                <button onClick={() => navigate("/teacher")} className={`w-full flex items-center gap-3 p-2 rounded-lg transition ${activeClass("/teachers")}`}><UsersIcon className="w-4 h-4"/>All Teachers</button>
                ...
                <button onClick={() =>
                    navigate("/teacher-allocate")} className={`w-full flex items-center gap-3 p-2 rounded-lg transition ${activeClass("/teacher-allocate")}`}><AdjustmentsVerticalIcon className="w-4 h-4"/>Teacher Allocation
                </button>
            </div>)}
        ...
        {openMenu === "subject" && (
            <div className="ml-8 mt-2 space-y-2">
                <button onClick={() => navigate("/subject")} className={`w-full flex items-center gap-3 p-2 rounded-lg transition ${activeClass("/teachers")}`}><BookOpenIcon className="w-4 h-4"/>All Subjects</button>
                <button onClick={() => navigate("/register-subject")} className={`w-full flex items-center gap-3 p-2 rounded-lg transition ${activeClass("/register-teacher")}`}><BookmarkSquareIcon className="w-4 h-4"/>Add Subject</button>
            </div>)}
    </aside>);
}